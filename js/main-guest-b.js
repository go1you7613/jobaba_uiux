(function () {
  ...
  // 대상별 추천 일자리(featured) — 대상 선택(#reco-target)에 따라 캐러셀 데이터셋을 교체한다
  ...
  // 비슷한 채용·정책(미니 카드) — 위 캐러셀과 같은 대상 선택을 공유한다
  var miniItemsByTarget = {
    youth: [
      { icon: "code-2", title: "프론트엔드 개발 청년인턴", meta: "경기 판교 · 인턴 · ~D-10", href: "detail-guest.html" },
      { icon: "wallet", title: "청년 노동자 통장 (경기)", meta: "현금성 · 월 10만원 매칭 · ~D-3", href: "detail-policy-guest.html" },
  ...
  function renderMini(target) {
    ...
    if (!row) return;
    if (heading) heading.textContent = "비슷한 " + targetLabel[target] + " 채용·정책";
    row.innerHTML = miniItemsByTarget[target].map(function (item) {
      ...
      return '<a class="vs-mini-card"' + hrefAttr + extAttrs + '>' +
        ...
        '<div class="mc-meta">' + item.meta + '</div></a>';
    }).join("");
    if (window.lucide) window.lucide.createIcons();
  }
